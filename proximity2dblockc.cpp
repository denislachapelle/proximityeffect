...
class ProximityEffect
{
   ...
   const char *mesh_file = "tworoundwires2d.msh";
   int order = 1;
   double freq = -1.0;
   int nbrwires = 2;
...
block_prec->SetDiagonalBlock(2, &gs5);
block_prec->SetDiagonalBlock(3, &gs6);
...
int ProximityEffect::PostPrecessing()
{
   ...
   Vector CoeffVector2(5);
   CoeffVector2 = 0.0;
   for(int wc=0; wc<nbrwires; wc++) CoeffVector[wc] = sigma_*(*x)[2*nbrdof+wc*2];
   ...
   real_t CoeffArray3[]={-omega_*sigma_, -omega_*sigma_, 0.0, 0.0, 0.0};
   Vector CoeffVector3(CoeffArray3, 5);
   K3.UpdateConstants(CoeffVector3);
   
   ProductCoefficient Ji1Coeff(K3, AzrGFCoeff);

   PWConstCoefficient Ji2Coeff;
   real_t CoeffArray4[]={sigma_*(*x)[2*nbrdof+1], sigma_*(*x)[2*nbrdof+3], 0.0, 0.0, 0.0};
   Vector CoeffVector4(CoeffArray4, 5);
